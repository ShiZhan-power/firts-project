...
std::vector<STrack> BYTETracker::predict()
{
    this->frame_id++;
    std::vector<STrack> output_stracks;
    ...
    {
        if (this->tracked_stracks[i].is_activated)
            tracked_stracks_ptrs.push_back(&this->tracked_stracks[i]);
    }

    // 2. 联合 Lost 目标一起预测 (保持 Kalman 状态连续性，防止重连时跳变)
    std::vector<STrack*> strack_pool = joint_stracks(tracked_stracks_ptrs, this->lost_stracks);

    // 3. 执行卡尔曼滤波预测 (更新 mean 和 covariance)
    STrack::multi_predict(strack_pool, this->kalman_filter);

    // 4. 同步预测后的状态到 tlwh (供外部使用)
    // 这一步很重要，因为 sendAngleData 依赖 tlwh/tlbr，而 predict 只更新了 mean
    for (auto* strack : strack_pool) {
        strack->static_tlwh();
        strack->static_tlbr();
    }

    // 5. 收集输出 (只输出当前处于 Tracked 状态的目标)
    for (int i = 0; i < this->tracked_stracks.size(); i++)
    {
        if (this->tracked_stracks[i].is_activated)
        {
