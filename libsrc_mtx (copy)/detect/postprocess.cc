...
static const char *const labels[5] = {"person", "bike", "car", "motor", "bus"};
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, int src_w, int src_h, float conf_threshold, float nms_threshold, std::vector<Object>& od_results)
{
    ...
    float scale_w = (float)src_w / (float)app_ctx->model_width;
    float scale_h = (float)src_h / (float)app_ctx->model_height;
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        float out_box_left   = (int)(clamp(x1, 0, model_in_w) * scale_w);
        float out_box_top    = (int)(clamp(y1, 0, model_in_h) * scale_h);
        float out_box_right  = (int)(clamp(x2, 0, model_in_w) * scale_w);
        float out_box_bottom = (int)(clamp(y2, 0, model_in_h) * scale_h);
        
        Object obj;
        obj.box = cv::Rect_<float>(out_box_left, out_box_top, out_box_right - out_box_left, out_box_bottom - out_box_top);
        obj.score = obj_conf;
        obj.classId = id;
        od_results.push_back(obj);
    }
    return 0;
}
